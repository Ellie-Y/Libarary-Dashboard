...
// When search books via author, return the book data as well
function findAuthorBook(model, params, res) {
    model.findAll({ where: params, include: [db.Book] }).then(function(results) {
        if (results) {
            ret.json(results, res);
        } else {
            res.end();
...
router.get("/", function(req, res) {
    if (req.query.type.toLowerCase() === "book") {
        ...
        findAuthorBook(db.Author, getSearchParams(req.query, ["name"]), res);
